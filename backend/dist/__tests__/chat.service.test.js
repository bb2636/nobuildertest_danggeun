...
describe('chatService', () => {
    ...
    describe('getOrCreateRoom', () => {
        it('게시글이 없으면 404', async () => {
            ...
            await expect(chat_service_1.chatService.getOrCreateRoom(1, 999)).rejects.toMatchObject({
                message: '게시글을 찾을 수 없습니다.',
                statusCode: 404,
            });
        });
        it('본인 게시글이면 400', async () => {
            postRepository.findUserIdById.mockResolvedValue(1);
            await expect(chat_service_1.chatService.getOrCreateRoom(1, 10)).rejects.toMatchObject({
        ...
        it('이미 방이 있으면 해당 roomId 반환', async () => {
            ...
            chatRepository.findRoomByPostAndMembers.mockResolvedValue(5);
            const result = await chat_service_1.chatService.getOrCreateRoom(1, 10);
            expect(result).toEqual({ roomId: 5 });
            expect(chatRepository.createRoom).not.toHaveBeenCalled();
        });
        it('방이 없으면 생성 후 roomId 반환', async () => {
            postRepository.findUserIdById.mockResolvedValue(2);
            chatRepository.findRoomByPostAndMembers.mockResolvedValue(null);
            chatRepository.createRoom.mockResolvedValue(3);
            chatRepository.addMember.mockResolvedValue(undefined);
            const result = await chat_service_1.chatService.getOrCreateRoom(1, 10);
    ...
    describe('sendMessage', () => {
        it('방 멤버가 아니면 403', async () => {
            chatRepository.isMember.mockResolvedValue(false);
            await expect(chat_service_1.chatService.sendMessage(1, 99, '안녕')).rejects.toMatchObject({
        ...
        it('내용이 비어 있으면 400', async () => {
            ...
            await expect(chat_service_1.chatService.sendMessage(1, 1, '   ')).rejects.toMatchObject({
