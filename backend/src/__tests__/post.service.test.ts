...
describe('postService', () => {
  ...
  describe('getList', () => {
    it('목록과 total, page, totalPages 반환', async () => {
      (postRepository.findList as jest.Mock).mockResolvedValue({
        rows: [
          {
            id: 1,
            title: '제목',
      ...
      const result = await postService.getList({ page: 1, limit: 20 });
  ...
  describe('getDetail', () => {
    it('skipViewIncrement가 true면 조회수 증가 안 함', async () => {
      ...
      const result = await postService.getDetail(1, true);
    ...
    it('skipViewIncrement가 false면 조회수 +1', async () => {
      ...
      const result = await postService.getDetail(1, false);
